...
# Permission for API Gateway to invoke getUserData Lambda
# Without this, API Gateway can't call the Lambda function
resource "aws_lambda_permission" "api_gateway_invoke_get_user_data" {
  statement_id  = "AllowAPIGatewayInvokeGetUserData"
...
# Package the createHabit Lambda function code into a zip file
data "archive_file" "create_habit_zip" {
  type        = "zip"
  source_dir  = "${path.module}/../backend/createHabit"
  output_path = "${path.module}/../backend/createHabit.zip"
  
  excludes = ["*.zip", ".git"]
}

# Lambda function for createHabit
resource "aws_lambda_function" "create_habit" {
  filename         = data.archive_file.create_habit_zip.output_path
  function_name    = "${var.project_name}-create-habit"
  role            = aws_iam_role.lambda_execution.arn
  handler         = "index.handler"
  source_code_hash = data.archive_file.create_habit_zip.output_base64sha256
...
# API Gateway resource for /habits endpoint
resource "aws_api_gateway_resource" "habits" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  parent_id   = aws_api_gateway_rest_api.main.root_resource_id
  path_part   = "habits"
}

# API Gateway method (POST) for /habits
resource "aws_api_gateway_method" "create_habit" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
...
# OPTIONS method for CORS preflight requests for /habits
resource "aws_api_gateway_method" "options_habits" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
  resource_id   = aws_api_gateway_resource.habits.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
# Integration between API Gateway and createHabit Lambda
resource "aws_api_gateway_integration" "create_habit" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  resource_id = aws_api_gateway_resource.habits.id
  http_method = aws_api_gateway_method.create_habit.http_method
  
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.create_habit.invoke_arn
}

# Mock integration for OPTIONS (CORS preflight) for /habits
resource "aws_api_gateway_integration" "options_habits" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  resource_id = aws_api_gateway_resource.habits.id
  http_method = aws_api_gateway_method.options_habits.http_method
...
# Method response for OPTIONS - /habits
resource "aws_api_gateway_method_response" "options_habits" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  resource_id = aws_api_gateway_resource.habits.id
  http_method = aws_api_gateway_method.options_habits.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "options_habits" {
  ...
  resource_id = aws_api_gateway_resource.habits.id
  http_method = aws_api_gateway_method.options_habits.http_method
  status_code = aws_api_gateway_method_response.options_habits.status_code
  ...
  depends_on = [aws_api_gateway_integration.options_habits]
}

# Permission for API Gateway to invoke createHabit Lambda
resource "aws_lambda_permission" "api_gateway_invoke_create_habit" {
